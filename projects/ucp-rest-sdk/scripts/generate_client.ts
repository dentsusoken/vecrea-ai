...
const { generateZodClientFromOpenAPI, getHandlebars } = await import(
  "openapi-zod-client"
);
...
function assignTagsByAlias(openApiDoc: { paths?: Record<string, Record<string, unknown>> }) {
  ...
  for (const pathKey of Object.keys(paths)) {
    ...
    for (const method of methods) {
      const op = pathItem[method] as { tags?: string[]; operationId?: string } | undefined;
...
async function main() {
  ...
  const outputPath = process.argv[3] ?? resolve(process.cwd(), DEFAULT_OUTPUT_DIR);
